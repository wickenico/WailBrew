...
// the underlying error, and the full captured stderr text (trimmed lines,
// newline-joined) for callers that need to inspect it for known error
// patterns (e.g. "app already exists", "untrusted tap").
func runStreamingCommand(cmd *exec.Cmd, onStdout, onStderr func(line string)) (phase streamPhase, err error, stderrText string) {
	...
	if waitErr := cmd.Wait(); waitErr != nil {
		return phaseRun, waitErr, stderrOutput.String()
	}
	return phaseNone, nil, stderrOutput.String()
}
