...
export function mapToSupportedLanguage(lng?: string | null): SupportedLanguage {
    ...
    const exactMatch = supportedLanguages.find((lang) => lang.toLowerCase() === lowerNormalized);
    if (exactMatch) {
        return exactMatch;
    }

    const base = lowerNormalized.split("_")[0];
    const baseMatch = supportedLanguages.find((lang) => lang.toLowerCase().startsWith(base));
